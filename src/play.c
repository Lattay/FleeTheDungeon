...
static void load_level(int level_num){
  ...
  int err = level_load(layout_name, &data->level, &data->enemies, &data->n_enemies, &data->x, &data->y); 

  if(err){
    TraceLog(LOG_ERROR, TextFormat("Cannot load level %d (%s): %s", level_num, layout_name, level_msg_error(err)));
    exit(1);
  }
  ...
  for(int x = x0, i = 0; i < w; ++x, ++i){
    for(int y = y0, j = 0; j < h; ++y, ++j){
      switch(get_tile(&data->level, x, y)){
        ...
        case LOCKED_DOOR:
          DrawRectangle(ox + 2 + i * 50 - data->map_ox, oy + j * 50 - data->map_oy, 46, 50, YELLOW);
          DrawRectangle(ox + 2 + i * 50 - data->map_ox, oy + j * 50 - data->map_oy + 20, 46, 10, BLACK);
          break;
        case BLOCK:
